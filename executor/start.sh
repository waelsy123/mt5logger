...
# MT5 reads this from the Config directory on startup
echo "[Executor] Configuring MT5 for server: ${MT5_SERVER}, login: ${MT5_LOGIN}"
mkdir -p "$MT5_DIR/Config"
# Use printf to write UTF-16LE (MT5 config format)
python3 -c "
import os
server = os.environ.get('MT5_SERVER', '')
login = os.environ.get('MT5_LOGIN', '0')
password = os.environ.get('MT5_PASSWORD', '')
# Write common.ini with server/login info (UTF-16LE with BOM)
